 ...
 */
public class Repository extends AbstractCommand {

    private static final ArgumentType[] ARGUMENTS = new ArgumentType[] { ArgumentType.STRING };

    public Repository() {
        super("repository", new String[] {}, "repository <hippo|jcr>",
                "set the repository type: hippo or jcr. use logout, login to change session", ARGUMENTS);
    }
    ...
    protected final boolean executeCommand(final String[] args) {
        if ("hippo".equals(args[1])) {
            JcrWrapper.setHippoRepository(true);
        } else if ("jcr".equals(args[1])) {
            JcrWrapper.setHippoRepository(false);
        } else {
            JcrShellPrinter.printWarnln("Unknown repository type: " + args[1]);
            return false;
        }
        return true;
    }

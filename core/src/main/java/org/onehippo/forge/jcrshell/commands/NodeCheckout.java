 ...
 */
public class NodeCheckout extends AbstractCommand {

    public NodeCheckout() {
        super("nodecheckout", new String[] { "checkout" }, "checkout", "check out the current node");
    }
    ...
    protected final boolean executeCommand(final String[] args) throws RepositoryException {
        ...
        if (!node.isCheckedOut()) {
            node.checkout();
            JcrShellPrinter.println("Node checked out.");
            return true;
        } else {
            JcrShellPrinter.printWarnln("Node alread checked out.");
            return false;
        }
    }

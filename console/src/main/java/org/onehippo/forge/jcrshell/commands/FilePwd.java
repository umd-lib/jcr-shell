...
/**
 * Show the working directory on the file system.
 */
public class FilePwd extends AbstractCommand {

    private static final ArgumentType[] ARGUMENTS = new ArgumentType[] { ArgumentType.DIRECTORY };

    public FilePwd() {
        super("filepwd", new String[] { "lpwd" }, "filepwd <path>", "change the working directory on the file system",
                ARGUMENTS);
    }
    ...
    protected final boolean executeCommand(final String[] args) {
        JcrShellPrinter.println(FsWrapper.getCwd().getAbsolutePath());
        return true;
    }
